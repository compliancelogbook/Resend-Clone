...
export default function FooterWatermark() {
  ...
  function updateGlow(clientX: number, clientY: number) {
    ...
    gradient.setAttribute("cx", x.toFixed(1));
    gradient.setAttribute("cy", y.toFixed(1));
  }

  return (
    <svg
      ref={svgRef}
      className={`footer-watermark ${hovering ? "is-hot" : ""}`}
    ...
    >
      <defs>
        <radialGradient ref={gradientRef} id="footer-watermark-glow" gradientUnits="userSpaceOnUse" cx={VB_W / 2} cy={VB_H / 2} r={GLOW_R}>
          <stop offset="0%" stopColor="rgba(255,255,255,.95)" />
          <stop offset="35%" stopColor="rgba(210,255,238,.55)" />
          <stop offset="100%" stopColor="rgba(84,246,194,0)" />
        </radialGradient>
        <filter id="footer-watermark-bloom" x="-60%" y="-60%" width="220%" height="220%">
      ...
      <path className="footer-watermark-base" d={WORDMARK_PATH} />
      <path className="footer-watermark-glow-path" d={WORDMARK_PATH} filter="url(#footer-watermark-bloom)" />
    </svg>
  );
}
